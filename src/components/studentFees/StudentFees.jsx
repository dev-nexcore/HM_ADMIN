...
const StudentFees = () => {
  ...
  // ── NEW: Invoice detail modal state ─────────────────────────────────────────
  ...
  const studentFeeStats = useMemo(() => students.map(student => {
    ...
    const totalFees = studentInvoices.reduce((s, i) => s + i.amount, 0);
    const paidFees = studentInvoices.filter(i => i.status === "paid").reduce((s, i) => s + i.amount, 0);
    const pendingFees = totalFees - paidFees;
    const isOverdue = studentInvoices.some(i => i.status === "pending" && new Date(i.dueDate) < new Date());
    const studentName = student.studentName || `${student.firstName || ""} ${student.lastName || ""}`.trim() || "Unknown Student";
    ...
    return {
      ...
      status: pendingFees === 0 && totalFees > 0 ? "paid" : (isOverdue ? "overdue" : (pendingFees > 0 ? "pending" : "no_invoice")),
  ...
  const handleGenerateInvoice = async (e) => {
    ...
    try {
      ...
      fetchData();
      setInvoiceForm({ amount: "", invoiceType: "hostel_fee", dueDate: new Date().toISOString().split("T")[0], description: "" });
    } catch (error) {
  ...
  // ── UPDATED: Proper ledger download with KGF branded header ─────────────────
        ...
        <style>
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .kgf-header {
            ...
            padding-bottom: 20px;
            margin-bottom: 10px;
            border-bottom: 3px solid #3E4B28;
          }
          .kgf-left {
            display: flex;
            align-items: center;
          ...
          .doc-title-strip .dtitle {
            font-size: 15px;
            font-weight: 800;
            letter-spacing: 0.05em;
            text-transform: uppercase;
          }
          .doc-title-strip .dmeta {
            font-size: 12px;
            opacity: 0.75;
          }
      ...
      <body>
        ...
        <div class="student-section">
          <div>
            ...
            <div class="value">${activeStudent.studentName}</div>
          </div>
          <div>
            <div class="label">Student ID</div>
            <div class="value">${activeStudent.studentId || "—"}</div>
          </div>
          <div>
            <div class="label">Room / Bed</div>
            <div class="value">${activeStudent.roomBedNumber || "Unassigned"}</div>
          </div>
          <div>
            <div class="label">Total Invoices</div>
            <div class="value">${activeStudent.allInvoices.length}</div>
          </div>
        </div>
        ...
        <div style={css.glassCard} className="sf-fade-up">
          ...
          <div className="sf-hide-mobile">
            <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 8px" }}>
              <thead>
                <tr>
                  {["Student Identity", "Room/Bed", "Fee Summary", "Status", ""].map(h => (
                    <th key={h} style={{
              ...
              <tbody>
                {filteredStudents.map((s, i) => (
                  <tr key={s._id || i} className="sf-row" style={{ transition: "all 0.2s ease" }}>
                    ...
                    <td style={{ padding: "16px 20px", background: "#fff", borderTop: `1px solid ${T.border}`, borderBottom: `1px solid ${T.border}` }}>
                      <p style={{ fontWeight: 900, color: T.text, fontSize: 15, margin: 0 }}>{formatCurrency(s.totalFees)}</p>
                      <p style={{ fontSize: 11, fontWeight: 700, color: s.pendingFees > 0 ? T.orange : T.green, margin: 0 }}>
                        {s.pendingFees > 0 ? `${formatCurrency(s.pendingFees)} Due` : "No Dues"}
                      </p>
                    </td>
                    <td style={{ padding: "16px 20px", background: "#fff", borderTop: `1px solid ${T.border}`, borderBottom: `1px solid ${T.border}` }}>
                    ...
                    <td style={{ padding: "16px 20px", background: "#fff", borderRadius: "0 16px 16px 0", border: `1px solid ${T.border}`, borderLeft: "none", textAlign: "right" }}>
                      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                        ...
                        <button style={{ ...css.btnPrimary, padding: "10px 18px" }} onClick={() => { setSelectedStudentId(s._id); setShowGenerateModal(true); }}>
          ...
          <div className="sf-show-mobile" style={{ flexDirection: "column", gap: 16 }}>
            {filteredStudents.map((s, i) => (
              <div key={i} style={{ background: "#fff", borderRadius: 20, padding: 20, border: `1px solid ${T.border}`, boxShadow: `0 4px 12px ${T.shadow}` }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", background: "#F9FAFB", padding: 12, borderRadius: 12, marginBottom: 16 }}>
                  <div>
                    <p style={{ fontSize: 10, color: T.textMuted, fontWeight: 700 }}>TOTAL</p>
                    <p style={{ fontWeight: 800 }}>{formatCurrency(s.totalFees)}</p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontSize: 10, color: T.textMuted, fontWeight: 700 }}>OUTSTANDING</p>
                    <p style={{ fontWeight: 800, color: s.pendingFees > 0 ? T.orange : T.green }}>{formatCurrency(s.pendingFees)}</p>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <button style={{ ...css.btnPrimary, flex: 1, justifyContent: "center" }} onClick={() => { setSelectedStudentId(s._id); setShowGenerateModal(true); }}>New Bill</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {showLedgerModal && activeStudent && (
        <ModalOverlay onClose={() => { setShowLedgerModal(false); setSelectedInvoice(null); }}>
          <div className="sf-fade-up" style={{
            ...css.glassCard, width: "100%", maxWidth: 800, padding: 0,
                  ...
                  <div key={idx} className="inv-row" style={{
                    ...
                    <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                      <div style={{
                        ...
                        display: "flex", alignItems: "center", justifyContent: "center",
                        color: inv.status === "paid" ? T.green : T.orange
                      }}>
                    ...
                    <div style={{ textAlign: "right", display: "flex", alignItems: "center", gap: 16 }}>
                      <div>
                        <p style={{ fontWeight: 900, color: T.text, fontSize: 16, margin: 0 }}>{formatCurrency(inv.amount)}</p>
                        <p style={{
                          fontSize: 10, fontWeight: 800, textTransform: "uppercase", margin: 0,
                          color: inv.status === "paid" ? T.green : inv.status === "overdue" ? T.red : T.orange
                        }}>{inv.status}</p>
                      </div>
                      {/* ── EYE BUTTON ── */}
                      <button
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                <div>
                  ...
                  {selectedInvoice.description && (
                    <p style={{ fontSize: 13, opacity: 0.75, marginTop: 4, margin: "4px 0 0" }}>
                      {selectedInvoice.description}
                    </p>
                  )}
                </div>
            ...
            <div style={{ padding: 32 }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                ...
                {selectedInvoice.status !== "paid" && (
                  <button
                    disabled={submitting}
                    style={{ ...css.btnPrimary, flex: 2, justifyContent: "center", opacity: submitting ? 0.7 : 1 }}
